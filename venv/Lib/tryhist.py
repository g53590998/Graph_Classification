...
if __name__ == '__main__':
    original_img = cv2.imread('101_ObjectCategories/test_graph/118_0089.jpg')
    img = cv2.resize(original_img, None, fx=0.6, fy=0.6, interpolation=cv2.INTER_CUBIC)
    b, g, r = cv2.split(img)

    Purple = calcAndDrawHist(img, [128, 0, 128])


    histImgB = calcAndDrawHist(b, [255, 0, 0])
    histImgG = calcAndDrawHist(g, [0, 255, 0])
    histImgR = calcAndDrawHist(r, [0, 0, 255])
    aa=histImgG+histImgB+histImgR

    #cv2.imshow("histAll", aa)
    #cv2.imshow("histsb", Purple)
